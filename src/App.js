...
function App() {
  const [simpson, setSimpson] = React.useState(false);
  ...
  function simsponFamilyly() {
    setSimpson(!simpson);
  }

  return (
    <div>
      <button onClick={simsponFamilyly}>
        Simpson Family Only: {simpson ? 'ON' : 'OFF'}
      </button>

      {quoteList
        .filter((quote) => !simpson || quote.character.includes('Simpson'))
        .map((quote, index) => (
